...
void run(int argc, char** argv)
{
    ...
    gpu::gpu_mem_32u input_gpu(n), precount(n), prefix_sum_accum_gpu(n);
    ...
    for (int iter = 0; iter < 20; ++iter) {
        precount.writeN(as.data(), n); // не считаем время тк input_gpu.writeN(as.data(), n) тоже не учитывается
        timer t;
        
        unsigned int k = 1;
        // std::cout << "start" << std::endl;
        while (n >= (1u<<k)) {
            // std::cout << "start precount k = " << k << std::endl;
            cuda::prefix_sum_01_precount(gpu::WorkSize(128, div_ceil(n, 1u<<k)), input_gpu, precount, n, k);
            // std::cout << "end precount k = " << k << std::endl;
            ++k;
            for (int ii = 0; ii < FUSE_N; ++ii) {
                ++k;
            }
        }
        // std::cout << "end all precount\nstart final" << std::endl;
        cuda::prefix_sum_02_final(gpu::WorkSize(128, n), precount, prefix_sum_accum_gpu, n);
    ...
    // std::vector<unsigned int> precount_cpu = precount.readVector();

    // std::cout << "precount:\n[";
    // for (size_t i = 0; i < n; ++i) {
    //     std::cout << precount_cpu[i] <<  ", ";
    // }
    // std::cout << "]" << std::endl;
