...
void run(int argc, char** argv)
{
    ...
    gpu::gpu_mem_32u reduction_buffer2_gpu(div_ceil(div_ceil(n, (unsigned int)GROUP_SIZE), (unsigned int)GROUP_SIZE));
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeCUDA) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        sum_accum_gpu.fill(0);
                        uint32_t n_r = div_ceil(n, (unsigned int)GROUP_SIZE);
                        uint32_t n_rr = div_ceil(n_r, (unsigned int)GROUP_SIZE);
                        cuda::sum_04_local_reduction(gpu::WorkSize(GROUP_SIZE, n), input_gpu, reduction_buffer1_gpu, n);
                        cuda::sum_04_local_reduction(gpu::WorkSize(GROUP_SIZE, n_r), reduction_buffer1_gpu, reduction_buffer2_gpu, n_r);
                        std::vector<uint32_t> sum_array(n_rr);
                        reduction_buffer2_gpu.readN(sum_array.data(), n_rr);
                        gpu_sum = 0;
